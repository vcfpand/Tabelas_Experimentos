...
import { toISOLocal, showError, hideError } from './utils.js';
...
export function initConfigPanel() {
  ...
  panel.innerHTML = `
    ...
    <div class="card"><div class="card-title">Aclimatação</div><div class="fg2">
      ...
      <div class="field"><p style="font-size:12px;color:var(--text2);line-height:1.7;padding-top:4px">O período vai da data escolhida até o Dia 0 (biometria inicial). Os dias serão numerados negativamente.</p></div>
    </div></div>
  ...
  function goStep1() {
    ...
    state.cfg = { startDate: sd, acclimStart: as, days: nd, boxes: nb, treats: nt, fishPerBox: fp, potWeights: {} };
    state.assigns = {}; for (let i=0; i<nt; i++) state.assigns[i] = [];
    state.confirmed = false; state.dailyData = []; state.bioData = []; state.bioDates = [];
    saveState();
    initTreatmentPanel();
    unlockPanel(1);
    setActivePanel(1);
